package com.example.xoliu_weather.ui;
...
import com.example.xoliu_weather.R;
import com.example.xoliu_weather.databinding.ActivityMainBinding;
import com.example.xoliu_weather.databinding.DialogDailyDetailBinding;
import com.example.xoliu_weather.databinding.DialogHourlyDetailBinding;
...
import com.example.xoliu_weather.location.GoodLocation;
import com.example.xoliu_weather.location.LocationCallback;
import com.example.xoliu_weather.ui.adapter.DailyAdapter;
import com.example.xoliu_weather.ui.adapter.HourlyAdapter;
import com.example.xoliu_weather.ui.adapter.LifestyleAdapter;
import com.example.xoliu_weather.utils.CityDialog;
import com.example.xoliu_weather.utils.EasyDate;
import com.example.xoliu_weather.utils.GlideUtils;
import com.example.xoliu_weather.utils.MVUtils;
import com.example.xoliu_weather.viewmodel.MainViewModel;
...
public class MainActivity extends NetworkActivity<ActivityMainBinding> implements LocationCallback, CityDialog.SelectedCityCallback {
        ...
        jumpActivityIntent = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
            if (result.getResultCode() == RESULT_OK && result.getData() != null) {
                ...
                if (city.equals(MVUtils.getString(Constant.LOCATION_CITY)) && cityFlag == 0) {
                    Log.d("xoliuTAG", "onRegister: 管理城市页面返回不需要进行天气查询" + city);
                    return;
                }
                //反之就直接调用选中城市的方法进行城市天气搜索
                Log.d("xoliuTAG", "onRegister: 管理城市页面返回进行天气查询" + city);
                selectedCity(city);
            }
        });
    }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (id == R.id.item_switching_cities) {
            if (cityDialog != null) {cityDialog.show();}
        }else if (id == R.id.item_relocation){
            startLocation();//点击重新定位时，再次定位
        } else if (id == R.id.item_bing) {
        ...
        } else if (id == R.id.item_manage_city) {
    ...
    public void onReceiveLocation(BDLocation bdLocation) {
        ...
        if (viewModel != null && district != null) {
            ...
            Log.d("xoliuTAG", "district: " + district);
        }
    }
